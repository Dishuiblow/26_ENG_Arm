...
**/
//TODO:将原两种电机反馈函数合并
void dm_motor_fdback(Joint_Motor_t *motor, uint8_t *rx_data,uint32_t data_len){
...
************************************************************************
**/
void mit_ctrl_4310(hcan_t* hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq){
	uint8_t data[8];
...
void mit_ctrl_4340(hcan_t* hcan, uint16_t motor_id, float pos, float vel,float kp, float kd, float torq)
{
	uint8_t data[8];
	uint16_t pos_tmp,vel_tmp,kp_tmp,kd_tmp,tor_tmp;
	uint16_t id = motor_id + MIT_MODE;
